...
async def start(message: types.Message):
    response = await InteractionBackendAPI.check_user_is_admin(message.from_user.id)
    if len(response) > 0:
        ikb = await create_start_ikb(response)
        await message.answer(
            '👋 Добро пожаловать! Я бот для управления корпоративными группами.\n\n'
            '📋 Вот что я могу:\n'
            ...
            '   - Удаление участников\n'
            '   - Ограничение доступа\n\n'
            '⚠️ Для корректной работы бота в группе предоставьте ему права администратора.',
            reply_markup=ikb
        )
    else:
        await message.answer('Вы не являетесь администратором ни одного из корпоративных чатов')
