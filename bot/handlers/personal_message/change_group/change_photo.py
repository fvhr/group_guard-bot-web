...
@dp.message_handler(content_types=types.ContentType.PHOTO, state=ChangeGroup.change_photo)
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(file_url) as response:
            if response.status == 200:
                ...
                input_file = types.InputFile(io.BytesIO(photo_bytes), filename='new_chat_photo.jpg')
