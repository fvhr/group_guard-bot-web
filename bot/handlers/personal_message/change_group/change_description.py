...
from keyboards.inlines import create_cancel_change_group_ikb, create_manage_chat_ikb
from loader import dp, bot
from modules.interaction_api import InteractionBackendAPI
from states.change_group import ChangeGroup


@dp.callback_query_handler(lambda call: call.data.startswith('change_description_'))
...
@dp.message_handler(content_types=types.ContentType.TEXT, state=ChangeGroup.change_description)
async def save_and_update_description(message: types.Message, state: FSMContext):
    data = await state.get_data()
    chat_id = data.get('chat_id')
    new_description = message.text
    await bot.set_chat_description(chat_id=chat_id, description=new_description)
    await InteractionBackendAPI.chats_patch(chat_id, {'description': new_description})
