...
async def delete_all_chat(call: types.CallbackQuery):
    ...
    for chat in chats:
        if chat['bot_is_admin']:
            try:
                await bot.kick_chat_member(chat_id=chat['id'], user_id=member_id)
                await InteractionBackendAPI.delete_users_chats(chat_id=chat['id'], user_id=member_id)
                messages.append(f'Пользователь успешно удалён из чата: {chat["title"]}✅')
            except UserIsAnAdministratorOfTheChat as ex:
                messages.append(f'Не удалось удалить пользователя из чата: {chat["title"]}❌\n'
    ...
    if bot_is_admin:
        ...
        await call.message.answer(message_send, parse_mode='HTML', reply_markup=ikb)
    else:
