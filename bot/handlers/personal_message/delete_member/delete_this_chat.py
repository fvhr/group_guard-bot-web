...
async def delete_this_chat(call: types.CallbackQuery):
    ...
    try:
        ...
        await InteractionBackendAPI.delete_users_chats(chat_id=chat_id, user_id=member_id)
        await bot.edit_message_text(f'Пользователь успешно удалён из чата: {chat.title}✅',
                                    message_id=call.message.message_id, chat_id=call.message.chat.id)
    except UserIsAnAdministratorOfTheChat:
        await bot.edit_message_text(f'Не удалось удалить пользователя из чата: {chat.title}❌\n'
